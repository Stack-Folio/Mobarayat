...
import { addArticle } from "@/utils/fetchData";
import { ArticleType } from "@/utils/Types";
import SuccessModal from "../ui/SuccessModal";
import { encode as encodeData } from "js-base64";

const AddNewBlog = ({ token }: { token: string }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    console.log(encodedArticle);

    const res = await addArticle({ data: encodedArticle, token });
  ...
  return (
    <AdminFormsContainer>
      ...
      <div>
        <AdminPageTitle>أضافة موضوع جديد</AdminPageTitle>
        <form className="my-10 space-y-3 text-[15px]">
          ...
          <div className="">
            <ContentAr setArticle={setArticle} />
          </div>
          <div className="">
            <ContentEn setArticle={setArticle} />
          </div>
